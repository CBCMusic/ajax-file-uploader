 ...
 *
 * Small modification made by ben.keen@gmail.com to add a custom abort() function
 */ 
...
ss.SimpleUpload.prototype = {
  ...
  */
  ...
  /**
  * Designate an element as a cancel button.
  */
  setCancelButton: function(elem) {
      this._cancelButton = ss.verifyElem(elem);
  },
  ...
  */
  _errorFinish: function(errorType, errorMsg, filename, response, progressBar, fileSizeBox, progressContainer) {
    ...
    this._settings.onError.call(this, filename, errorType, response, this.xhrManuallyAborted);
  ...
  */
  _uploadXhr: function() {
    ...
    /**
    * Aborts the current XHR request (if there is one).
    */
    self.xhrManuallyAborted = false;
    self.abort = function() {
        if (self._activeUploads > 0) {
            this.xhrManuallyAborted = true;
            xhr.abort();            
            self._settings.onAbort();
        }
    },
